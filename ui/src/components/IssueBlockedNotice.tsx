...
function SuccessfulRunRetryNowControl({
  ...
  const scheduleLabel = relative === "now"
    ? t("issueDetailPage.blockedNotice.retryNow.dueNow")
    : relative
      ? t("issueDetailPage.blockedNotice.retryNow.scheduledWithEta", { eta: relative })
      : t("issueDetailPage.blockedNotice.retryNow.scheduled");
    ...
    <div className="mt-2 rounded-md border border-amber-300/70 bg-background/80 p-2 dark:border-amber-500/40 dark:bg-background/40">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div className="min-w-0 text-xs leading-5 text-amber-900 dark:text-amber-100">
          {t("issueDetailPage.blockedNotice.retryNow.description", { schedule: scheduleLabel })}
        </div>
        <Button
          type="button"
          variant="outline"
        ...
        >
          {retryNow.isPending ? (
            <span className="inline-flex items-center gap-1.5">
              ...
              {t("issueDetailPage.blockedNotice.retryNow.retrying")}
            </span>
          ) : success ? (
            <span className="inline-flex items-center gap-1.5">
              <CheckCircle2 className="h-3.5 w-3.5" aria-hidden="true" />
              {retryNow.data?.outcome === "already_promoted"
                ? t("issueDetailPage.blockedNotice.retryNow.alreadyPromoted")
                : t("issueDetailPage.blockedNotice.retryNow.promoted")}
            </span>
          ) : (
            <span className="inline-flex items-center gap-1.5">
              <RotateCcw className="h-3.5 w-3.5" aria-hidden="true" />
              {t("issueDetailPage.blockedNotice.retryNow.retryNow")}
            </span>
          )}
        </Button>
      </div>
  ...
  return (
    ...
    >
      <StatusGlyph
        ...
        size="sm"
        title={t("issueDetailPage.blockedNotice.liveWork.statusTitle", { status: waitingTaskStatusLabel(blocker.status) })}
      />
      ...
      {running ? (
        <span className="ml-0.5 rounded-full bg-blue-500/15 px-1.5 py-0.5 text-(length:--text-nano) font-medium uppercase tracking-wide text-blue-700 dark:bg-blue-400/20 dark:text-blue-200">
          {t("issueDetailPage.blockedNotice.liveWork.running")}
        </span>
      ) : null}
    </IssueLinkQuicklook>
  );
...
function WaitingOnLiveWorkNotice({
  ...
  const queuedNoun = total === 1
    ? t("issueDetailPage.blockedNotice.liveWork.task")
    : t("issueDetailPage.blockedNotice.liveWork.tasks");
      ...
      <div className="flex items-start gap-2">
        ...
        <div className="min-w-0 flex-1 space-y-2">
          <div className="space-y-1">
            <p className="font-medium leading-5">{t("issueDetailPage.blockedNotice.liveWork.title")}</p>
            <p className="leading-5">
              {t("issueDetailPage.blockedNotice.liveWork.description", { total, noun: queuedNoun })}
            </p>
          </div>

          <div className="space-y-1" data-testid="issue-blocked-notice-progress">
            <div className="text-xs font-medium text-blue-800 dark:text-blue-200">
              {t("issueDetailPage.blockedNotice.liveWork.progressDone", { done: doneCount, total })}
              {runningCount > 0 ? ` · ${t("issueDetailPage.blockedNotice.liveWork.progressRunning", { count: runningCount })}` : null}
            </div>
            <div
              role="progressbar"
              aria-label={t("issueDetailPage.blockedNotice.liveWork.progressAria")}
              aria-valuemin={0}
            ...
            >
              {steps.map(({ blocker, status }) => (
                <span
                  ...
                  title={`${blocker.identifier ?? blocker.id.slice(0, 8)}: ${t(`issueDetailPage.blockedNotice.liveWork.stepStatus.${status}`)}`}
                  aria-hidden
                />
              ))}
            </div>
            ...
            <div className="flex items-stretch gap-2">
              ...
              <div className="min-w-0 pb-0.5">
                <span className="inline-block rounded-md border border-dashed border-blue-300/70 px-2 py-1 text-xs text-blue-800 dark:border-blue-500/40 dark:text-blue-200">
                  {t("issueDetailPage.blockedNotice.liveWork.thisTaskResumes")}
                </span>
              </div>
            </div>
          </div>
          ...
          {nowRunning.length > 0 ? (
            ...
            >
              <div className="text-xs font-medium text-blue-800 dark:text-blue-200">
                {t("issueDetailPage.blockedNotice.liveWork.nowRunning")}
              </div>
...
export function IssueBlockedNotice({
  ...
  const blockerLabel = blockers.length === 1
    ? t("issueDetailPage.blockedNotice.linkedTask")
    : t("issueDetailPage.blockedNotice.linkedTasks");
  ...
  const responsibleName = agentName ?? t("issueDetailPage.composer.theAssignee");
    ...
    >
      <div className="flex items-start gap-2">
        ...
        <div className="min-w-0 space-y-1.5">
          {showSuccessfulRunHandoff ? (
            <>
              <p className="font-medium leading-5">{t("issueDetailPage.blockedNotice.nextStep.title")}</p>
              <p className="leading-5">
                {t("issueDetailPage.blockedNotice.nextStep.description")}
              </p>
              <ul className="list-disc space-y-1 pl-5 text-xs leading-5 text-amber-900 dark:text-amber-100">
                <li>{t("issueDetailPage.blockedNotice.nextStep.items.doneOrCancelled")}</li>
                <li>{t("issueDetailPage.blockedNotice.nextStep.items.reviewOrInput")}</li>
                <li>{t("issueDetailPage.blockedNotice.nextStep.items.recordBlocker")}</li>
                <li>{t("issueDetailPage.blockedNotice.nextStep.items.delegateOrContinue")}</li>
              </ul>
              <div className="flex flex-wrap gap-1.5 text-xs">
                ...
                <span className="rounded-md border border-amber-300/70 bg-background/80 px-2 py-1 text-amber-900 dark:border-amber-500/40 dark:bg-background/40 dark:text-amber-100">
                  {t("issueDetailPage.blockedNotice.nextStep.askedAssignee", { name: agentName ?? t("issueDetailPage.composer.theAssignee") })}
                </span>
              </div>
              {successfulRunHandoff.detectedProgressSummary ? (
                <p className="text-xs leading-5 text-amber-800 dark:text-amber-200">
                  {t("issueDetailPage.blockedNotice.nextStep.detectedProgress", { summary: successfulRunHandoff.detectedProgressSummary })}
                </p>
              ) : null}
          ...
          {blockers.length > 0 || issueStatus === "blocked" ? (
            <>
                ...
                {blockers.length > 0
                  ? isStalled
                    ? stalledLeafBlockers.length > 1
                      ? <>{t("issueDetailPage.blockedNotice.messages.stalledMultiple", { blockerLabel })}</>
                      : <>{t("issueDetailPage.blockedNotice.messages.stalledSingle", { blockerLabel })}</>
                    : reopenSuppressed
                      ? <>{t("issueDetailPage.blockedNotice.messages.reopenSuppressed", { blockerLabel, verb: blockers.length === 1 ? t("issueDetailPage.blockedNotice.verbs.itIs") : t("issueDetailPage.blockedNotice.verbs.theyAre"), responsibleName })}</>
                      : <>{t("issueDetailPage.blockedNotice.messages.blockedUntilComplete", { blockerLabel, verb: blockers.length === 1 ? t("issueDetailPage.blockedNotice.verbs.itIs") : t("issueDetailPage.blockedNotice.verbs.theyAre") })}</>
                  : <>{t("issueDetailPage.blockedNotice.messages.blockedUntilTodo")}</>}
              </p>
              {reopenSuppressed && reopenSuppressedLeafId ? (
                ...
                >
                  {t("issueDetailPage.blockedNotice.messages.stillBlockedBy")}{" "}
                  ...
                  {reopenSuppressedOtherCount > 0
                    ? ` ${t("issueDetailPage.blockedNotice.messages.andOtherTasks", { count: reopenSuppressedOtherCount })}`
                    : null}
                  .
                </p>
              ) : null}
              ...
              {showStalledRow ? (
                <div className="flex flex-wrap items-center gap-1.5 pt-0.5">
                  <span className="text-xs font-medium text-amber-800 dark:text-amber-200">
                    {t("issueDetailPage.blockedNotice.messages.stalledInReview")}
                  </span>
                ...
                <div className="flex flex-wrap items-center gap-1.5 pt-0.5">
                  <span className="text-xs font-medium text-amber-800 dark:text-amber-200">
                    {t("issueDetailPage.blockedNotice.messages.ultimatelyWaitingOn")}
                  </span>
              ...
              {showParkedRow ? (
                ...
                >
                  <span className="inline-flex items-center gap-1 text-xs font-medium text-amber-800 dark:text-amber-200">
                    <Flag className="h-3 w-3" aria-hidden />
                    {t("issueDetailPage.blockedNotice.liveWork.blockedByParkedWork")}
                  </span>
