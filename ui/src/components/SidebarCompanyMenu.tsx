...
import { changeLocale, i18n, useTranslation } from "@/i18n";
    ...
    >
      ...
      {isEditing ? (
        <button
          ...
          aria-label={t("companyMenu.reorderNamed", {
            defaultValue: `Reorder ${company.name}`,
            name: company.name,
          })}
      ...
      <DropdownMenuTrigger asChild>
        <Button
          ...
          aria-label={currentName
            ? t("companyMenu.openSwitcherNamed", {
                defaultValue: `Open ${currentName} ${switcherNoun} switcher`,
                name: currentName,
              })
            : t("companyMenu.openSwitcher", { defaultValue: `Open ${switcherNoun} switcher` })}
        >
          <span className="flex min-w-0 flex-1 items-center gap-2">
            ...
            >
              {currentName ?? (isCloud ? "Select organization" : t("companyMenu.selectCompany", { defaultValue: "Select company" }))}
            </span>
          </span>
      ...
      <DropdownMenuContent align="start" sideOffset={8} className="w-64 p-1">
        <div className="flex items-center justify-between gap-2 px-2 py-1.5">
          <DropdownMenuLabel className="p-0 text-(length:--text-micro) font-semibold uppercase text-muted-foreground">
            {isCloud ? "Switch organization" : t("companyMenu.switchCompany", { defaultValue: "Switch company" })}
          </DropdownMenuLabel>
          ...
          {isCloud ? (
            ...
            <>
              ...
              {orderedCompanies.length === 0 ? (
                <DropdownMenuItem disabled>{t("companyMenu.noCompanies", { defaultValue: "No companies" })}</DropdownMenuItem>
              ) : null}
            </>
          )}
        </div>
        ...
        {isCloud && !createStackUrl ? null : (
          <>
            ...
            >
              ...
              <span>{isCloud ? "Create new organization..." : t("companyMenu.createNewCompany", { defaultValue: "Create new company..." })}</span>
            </DropdownMenuItem>
            <DropdownMenuSeparator />
          </>
        )}
        <DropdownMenuItem asChild disabled={isEditingOrder}>
          ...
          >
            ...
            <span className="truncate">
              {currentName
                ? t("companyMenu.invitePeopleNamed", { defaultValue: `Invite people to ${currentName}`, name: currentName })
        ...
        <DropdownMenuItem asChild disabled={isEditingOrder}>
          ...
          >
            ...
            <span>{isCloud ? "Organization settings" : t("companyMenu.companySettings", { defaultValue: "Company settings" })}</span>
          </Link>
        </DropdownMenuItem>
        <DropdownMenuSeparator />
        <DropdownMenuLabel className="px-2 py-1.5 text-(length:--text-micro) font-semibold uppercase text-muted-foreground">
          {t("companyMenu.language", { defaultValue: "Language" })}
        </DropdownMenuLabel>
        <DropdownMenuItem onSelect={() => void changeLocale("pt-BR")} disabled={isEditingOrder}>
          <span className="flex min-w-0 flex-1 items-center gap-2">
            <span>{t("companyMenu.portugueseBrazil", { defaultValue: "Português (Brasil)" })}</span>
            {i18n.language === "pt-BR" ? <Check className="size-4 text-muted-foreground" /> : null}
          </span>
        </DropdownMenuItem>
        <DropdownMenuItem onSelect={() => void changeLocale("en")} disabled={isEditingOrder}>
          <span className="flex min-w-0 flex-1 items-center gap-2">
            <span>{t("companyMenu.english", { defaultValue: "English" })}</span>
            {i18n.language === "en" ? <Check className="size-4 text-muted-foreground" /> : null}
          </span>
        </DropdownMenuItem>
        {session?.session ? (
          <>
            ...
            >
              ...
              <span>{signOutMutation.isPending
                ? t("companyMenu.signingOut", { defaultValue: "Signing out..." })
                : t("companyMenu.signOut", { defaultValue: "Sign out" })}</span>
            </DropdownMenuItem>
          </>
        ) : null}
      </DropdownMenuContent>
