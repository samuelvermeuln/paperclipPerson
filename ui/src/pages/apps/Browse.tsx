...
export function Browse() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      ...
      { label: t("apps.browse.title", { defaultValue: "Browse" }) },
    ]);
    ...
    <div className="max-w-5xl space-y-8 pb-12">
      <header>
        <h1 className="text-2xl font-bold tracking-tight">
          {t("apps.browse.title", { defaultValue: "Browse" })}
        </h1>
          ...
          <section className="space-y-3">
            <div className="text-(length:--text-micro) font-semibold uppercase tracking-wide text-muted-foreground">
              {trimmed
                ...
                : t("apps.browse.allApps", { defaultValue: "All apps" })}
            </div>
            {filtered.length === 0 ? (
...
function AppTile({
  ...
  const actionLabel = connectedCount > 0
    ? t("apps.browse.connectedAlready", { defaultValue: "{{count}} connected already", count: connectedCount })
    : disabled
